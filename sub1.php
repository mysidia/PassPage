<?php
  ...
  if (!empty($_POST['c']) && !empty($_POST['c'])) {
   ...
   if (!empty($_REQUEST['countermax'])) {
        $countermax = intval($_REQUEST['countermax']);
   }
   ...
   if (!empty($_REQUEST['expire'])) {
        $e = 86400 * intval($_REQUEST['expire']);
        if ($e < 86400) { $e = 86400; }
   }
  ...
  ini_set("sendmail_from",  $CRED_sender);
   ...
   mail($recipient, 'Credential Link', $VIEW_baseurl . $o . '&k=' . base64_encode($rankey), 'From: <' . $CRED_sender . '>\r\n', '-f' . $CRED_sender);

   syslog( LOG_NOTICE,  'Sent link to '  . $VIEW_baseurl . $o . '&k=' . base64_encode($rankey) );
   echo 'Credential Link has been sent<BR><BR>'; 

  } else {
?>
    <BODY><DIV style="border: solid 1px; width: 60%; padding: 50px 50px 50px 50px;"><FORM METHOD="POST" ACTION="sub1.php?kc=<?php echo $_GET['kc'];   ?>">
